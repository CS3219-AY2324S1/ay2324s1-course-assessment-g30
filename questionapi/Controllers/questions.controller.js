...
const {webScrapperQuestionDescription} = require('../Utils/webScrapperQuestionDescription.js')
...
const readQuestionDescriptionController = async (req,res,next) => {
    ...
    try {
        const doc = await QuestionDescriptionModel.findOne({"question_id" : reqQuestionid});
        if (doc == null) {
...
const addQuestionController = async (req,res,next) => {
    ...
    try {
        ...
        const documentWithHighestIndex = await QuestionModel.find().sort({question_id:-1}).limit(1)
...
const updateQuestionController = async (req,res,next) => {
    ...
    const original_document = await QuestionModel.findOne({"question_id" : question_id});
    ...
    try {
        const newQuestionDescription = await webScrapperQuestionDescription(new_link);
        await QuestionModel.updateOne({"question_id" : question_id}, {
            question_title: new_title,
            question_categories: new_category,
            question_complexity: new_complexity,
            question_link: new_link
        });
        await QuestionDescriptionModel.updateOne({"question_id" : question_id}, {
            question_description: newQuestionDescription
        });
...
module.exports = {readQuestionDescriptionController, readQuestionsController, addQuestionController, deleteQuestionController, updateQuestionController};